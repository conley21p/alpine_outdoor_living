...
interface HandOfCardsServicesProps {
  services?: ServiceData[];
}

export function HandOfCardsServices({ services = [] }: HandOfCardsServicesProps) {
  ...
  return (
    <div id="services" className="relative w-full bg-transparent overflow-hidden">
      {/* Intro Section */}
      <section className="services-intro py-20 lg:py-32 flex items-center justify-center bg-transparent">
        <div className="text-center px-6">
          ...
          <p className="mx-auto mt-5 max-w-3xl text-lg font-normal text-brand-textDark/70 sm:text-lg lg:text-2xl">
            Swipe to explore our professional solutions in {publicConfig.businessLocation}
          </p>
        </div>
      </section>
            ...
            {services.map((service, i) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Lava Lamp Blobs using brand colors */}
                  <div className="absolute top-[-20%] left-[-20%] w-[80%] h-[80%] rounded-full bg-brand-primary/20 blur-[60px] lava-lamp-slow-1 pointer-events-none z-0" />
                  <div className="absolute bottom-[10%] right-[-10%] w-[60%] h-[60%] rounded-full bg-brand-secondary/20 blur-[50px] lava-lamp-slow-2 pointer-events-none z-0" />
                  ...
                  <div className="relative z-10 p-5 lg:p-8 flex flex-col gap-5 lg:gap-8 h-full flex-grow pointer-events-none">
                    ...
                    <div className={`flex gap-3 pb-2 transition-opacity duration-300 ${isCenter ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
                      {publicConfig.hasServiceDetailPage && (
                        <Link href={`/services/${service.id}`} className="flex-1">
                          <button className="flex items-center justify-center gap-2 w-full py-4 px-4 font-bold text-brand-textDark bg-white/80 hover:bg-white border border-brand-primary/20 rounded-2xl transition-all active:scale-95 shadow-sm text-sm lg:text-base">
                            Explore
                          </button>
                        </Link>
                      )}
